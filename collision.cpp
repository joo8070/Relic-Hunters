...
namespace TTYONE_UTIL
{
	...
	bool isPtInRect(const MYRECT & rc, POINT pt)
	{
		RECT rc2;
		rc2.left = FLOAT_TO_INT(rc.left);
		rc2.top = FLOAT_TO_INT(rc.top);
		rc2.right = FLOAT_TO_INT(rc.right);
		rc2.bottom = FLOAT_TO_INT(rc.bottom);

		if(!PtInRect(&rc2,pt)) return false;
